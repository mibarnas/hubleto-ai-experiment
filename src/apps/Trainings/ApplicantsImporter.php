 ...
 * applicants' emails"), and attaches an Applicant per row to the order and
 ...
 */
class ApplicantsImporter extends \Hubleto\Erp\Core
{
  ...
  private function process(string $xlsxPath, ?int $idOrder): array
  {
    ...
    /** @var TrainingOrder */
    $mOrder = $idOrder ? $this->getModel(TrainingOrder::class)->record->find($idOrder) : null;
    $idTrainingDate = $mOrder?->id_training_date ?? 0;
    ...
    foreach ($rows as $row) {
      ...
      $existingApplicant = $mApplicant->record
        ->where('id_training_date', $idTrainingDate)
        ->where('id_worker', $idWorker)
        ->first();

      if (!$existingApplicant) {
        $mApplicant->record->recordCreate([
          'id_training_date' => $idTrainingDate,
          'id_worker' => $idWorker,
          'id_order' => $idOrder,
