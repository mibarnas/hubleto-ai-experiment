...
export default class FormSchedule<P, S> extends FormAlgo<FormScheduleProps, FormScheduleState> {
  ...
  getMainTab() {
    return { uid: 'default', title: <b>{this.translate('Schedule')}</b> };
  }

  getRelatedTabs() {
    return [ { uid: 'attendees', title: this.translate('Attendees') } ];
  }
            ...
            <div className='card-body'>
              {R.id > 0 ? <div className='flex flex-wrap gap-2'>
                ...
                <button
                  ...
                  onClick={() => this.bulkSend('trainings/api/send-questionnaire', this.translate('The catalog sheet and questionnaire links have been sent to the attendees.'))}
                >
                  <span className='icon'><i className='fas fa-clipboard-question'></i></span>
                  <span className='text'>{this.translate('Send catalog sheet and questionnaire')}</span>
                </button>
              ...
              <div className='text-xs text-gray-500 mt-2'>
                {this.translate('The meeting link goes out as one email addressed to every attendee. Every send is written to log/emails.')}
              </div>
            </div>
          </div>
        </div>;
